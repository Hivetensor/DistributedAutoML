...
from dml.utils import calculate_tree_depth, set_seed
    ...
    def load_checkpoint(self, checkpoint_file):
        ...
        for expr_str, fitness_values in population_data:
            ind = creator.Individual(
                SafePrimitiveTree.from_string(expr_str, self.pset, safe_eval)
            )
        ...
        for generation in tqdm(range(start_generation, self.config.Miner.generations)):
            ...
            for i, ind in enumerate(population):
                if not ind.fitness.valid:
                    ind.fitness.values = self.toolbox.evaluate(
                        ind, train_loader, val_loader
                    )
                logging.debug(
            ...
            fitnesses = map(
                ...
                invalid_ind,
                [train_loader] * len(invalid_ind),
                [val_loader] * len(invalid_ind),
            )
            ...
            for i, ind in enumerate(invalid_ind):
                ...
                logging.debug(
                    f"Gen {generation}, New Individual {i}: Fitness = {ind.fitness.values[0]}"
                )
            ...
            fits = [
                ind.fitness.values[0]
                for ind in population
                if not math.isinf(ind.fitness.values[0])
            ]
            ...
            if (
                ...
                self.push_to_remote(
                    best_individual_all_time,
                    f"{generation}_{best_individual_all_time.fitness.values[0]:.4f})",
                )
            ...
            logging.info(
                f"Generation {generation}: Best accuracy = {best_individual.fitness.values[0]:.4f}"
            )
    ...
    def run_island(
        ...
        for generation in range(self.config.Miner.generations):
            ...
            for ind in offspring:
                if not ind.fitness.valid:
                    ind.fitness.values = self.create_n_evaluate(
                        ind, train_loader, val_loader
                    )
    ...
    def mine(self):
        try:
            ...
            while any(p.is_alive() for p in processes):
                ...
                if generation > 0:
                    logging.info(
                        f"Best overall fitness:{best_overall.fitness.values[0]}"
                    )
                try:
                    candidates = []
                    try:
